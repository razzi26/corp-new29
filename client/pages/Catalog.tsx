  ...
  return (
    <div className="container mx-auto px-4 py-16 text-white">
      ...
      <div className="mt-8 grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {categories.map((c) => (
          <div key={c.title} className="group overflow-hidden rounded-2xl border border-white/20 bg-white/5 backdrop-blur">
            <div className="relative h-44 bg-gradient-to-r from-[hsl(var(--brand-start))] to-[hsl(var(--brand-end))]">
              ...
              <span className="absolute top-3 left-3 text-xs rounded-full bg-white/90 text-[hsl(var(--brand-end))] px-2 py-1 font-semibold">{c.tag}</span>
            </div>
            <div className="p-5">
              <h3 className="font-semibold text-lg">{c.title}</h3>
              <p className="mt-1 text-sm text-white/80">Contact our team for pricing and availability.</p>
              <div className="mt-4 flex gap-2">
                <a href="/contact" className="inline-flex items-center rounded-lg bg-white text-[hsl(var(--brand-end))] px-3 py-2 text-sm font-semibold shadow">Request Quote</a>
                <a href="/contact" className="inline-flex items-center rounded-lg border border-white/30 px-3 py-2 text-sm font-semibold hover:bg-white/10">Consultation</a>
              </div>
            </div>
          </div>
        ))}
